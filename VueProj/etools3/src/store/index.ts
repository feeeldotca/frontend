...
export default createStore({
  state: {
    ...
    rules: [
      (value:number) => (value && RegExp("^[0-9]+.*$").test(value)) || "Positive number only",
    ],
    // get totalLoads() {
    //   return totalLoads;
    // },
    // set totalLoads(value:number) {
    //   totalLoads = value;
    // },
    tables: [
      { //table[0]"table2"
      ...
      {//table[2]"table39" deleted from 2021 code book
         name: "table39",
         description: "Overcurrent device rating, A (deleted from 2021 code book)"
      //   cols: 5,
  ...
  getters: {
    ...
    getTable: (state) => (table:string, temperature:string) => {
    ...
    getLength: (state) => (size:string, current:string) => {
      // table[5]: table D3 on CEC code book
    ...
    getMaxCurrentPerSize: ((state, getters) => (size:string, temp:string) => {
    ...
    getLeastAwg: (state, getters) => (table:string, temperature:string) => {
    ...
    // get feeder size according to its current value from table 39 or tables[2]
    getFeederAwg: (state) => {
      (amp:string, tempInsulation:string)=>{
        const table2 = state.tables[0]
        const table4 = state.tables[1]
        //find ampacity column for the insulation temprature
        let ampTable2=table2.degree.find(tmp=>tmp.temp==tempInsulation)
        let ampTable4=table4.degree.find(tmp=>tmp.temp==tempInsulation)
        //find index for certain current from the above result
        let index2 = ampTable2.findIndex( i => i == amp)
        let index4 = ampTable4.findIndex( i => i == amp)
        //result of the feeder size should then be got from size table
        return {table2.awg_size[index2], table4.awg_size[index4]}

      }
    }
    //(amp:number) => Object.keys(tables[0]).find(a => a.copper_loads_amp > amp)
  },
  mutations: {
